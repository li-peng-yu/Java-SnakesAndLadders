...
public class Gameengine {
    ...
    public void setCurrentPlayerTurn(boolean isPlayerOne) {
        if (players.isEmpty()) return; // 如果没有玩家，则不执行任何操作
        String targetPlayerName = isPlayerOne ? "Player 1" : "Player 2"; // 根据布尔值确定目标玩家名称
        for (int i = 0; i < players.size(); i++) {
            if (players.get(i).name.equals(targetPlayerName)) { // 查找匹配名称的玩家
                this.currentPlayerIndex = i; // 设置当前玩家索引
                return;
            }
        }
        // 回退逻辑：如果目标玩家未找到（例如单人模式下isPlayerOne为false），
        // 或者为了确保currentPlayerIndex总是有合理的值。
        if (!isPlayerOne && singlePlayerMode) { // 在单人模式下，如果逻辑正确，isPlayerOne通常应为true
            this.currentPlayerIndex = 0; // 默认指向玩家1 (索引0)
        } else if (isPlayerOne) {
            this.currentPlayerIndex = 0; // 如果是玩家1，则指向索引0
        }
        // 注意：在双人模式下，如果isPlayerOne为false但未找到"Player 2"，则currentPlayerIndex可能不会被正确设置。
        // 确保玩家名称 ("Player 1", "Player 2") 与引擎初始化时使用的名称一致。
    }
    // 道具枚举
    public enum Item {
